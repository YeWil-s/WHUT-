...
int Encode(const char* fileName, const HuffmanCode cood, char* pBuffer, const int nSize)
{
	...
	if (strlen(cd) > 0)//若还有剩余字符串则重复上述步骤
	{
		pBuffer[pos++] = Str2byte(cd);
	}
	fclose(in);
...
int Compress(const char* fileName)
{
	...
	char* pbuff = NULL;
	pbuff = (char*)malloc(nSize * sizeof(char));
	memset(pbuff, 0, (nSize) * sizeof(char));
